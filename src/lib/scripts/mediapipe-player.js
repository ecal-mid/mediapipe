...
class MediaPipePlayer extends EventBus {
  ...
  stopRecord() {
    if (!this.status.is("idle")) this.status.emit("stopRecord");
  }
  ...
  update(pose) {
    ...
    if (duration > this.bufferMaxDuration) this.frames.shift();
  ...
  updateFrame() {
    ...
    if (this.playbackTime + deltaTime > timeNow + this.timeDrift) {
      this.requestFrame();
      return;
    }
...
function compress(pose) {
  ...
  const compressedPose = new Array(MediaPipePose.LANDMARK_ENTRIES.length).fill(
    null
  );
