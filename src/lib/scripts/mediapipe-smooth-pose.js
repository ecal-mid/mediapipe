...
class MediaPipeSmoothPose {
  ...
  target(skeleton) {
    ...
    if (!this.smoothedLandmarks) {
      ...
      for (let name in this.smoothedLandmarks) {
        this.velocities.set(
          this.smoothedLandmarks[name],
          this.constructor.createLandmark()
        );
      }
    }
  }
  ...
  smoothDamp() {
    ...
    this.onEachValue((storedValue, newValue, key, storedLandmark) => {
      ...
      const { result, velocity } = smoothDamp(
        storedValue,
        newValue,
        velocities[key],
        dampAmount,
  ...
  static DEFAULT_LANDMARK = Object.fromEntries(
    LANDMARK_KEYS.map((name) => [name, 0])
  );

  static createLandmarks() {
    return Object.fromEntries(
      LANDMARK_ENTRIES.map(([name]) => [name, this.createLandmark()])
    );
  }
